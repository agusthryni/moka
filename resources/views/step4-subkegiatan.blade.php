                                ...
                                <tbody>
                                    @foreach ($data_laporan as $laporan)
                                        <tr>
                                            ...
                                            <td>{{ $laporan->indikator_sub_kegiatan }}</td>
                                            <td>{{ $laporan->satuan_sub_kegiatan }}</td>
                                            <td>{{ $laporan->target_sub_kegiatan }}</td>
                                            <td>{{ $laporan->realisasi_kinerja_sub_kegiatan }}</td>
                                            <td>{{ $laporan->persen_kinerja_sub_kegiatan }}</td>
                                            <td>{{ $laporan->tenagaKerja->jumlah_tki_perempuan + $laporan->tenagaKerja->jumlah_tki_laki_laki + $laporan->tenagaKerja->jumlah_tenaga_kerja_asing }}</td>
                                            <td>{{ $laporan->pagu_sub_kegiatan }}</td>
                                            <td>{{ $laporan->realisasi_keuangan_sub_kegiatan }}</td>
                                            <td>{{ $laporan->persen_keuangan_sub_kegiatan }}</td>
                                            <td>{{ $laporan->keterangan_sub_kegiatan }}</td>
                                            <td>{{ $laporan->faktor_pendorong_sub_kegiatan }}</td>
                                            <td>{{ $laporan->faktor_penghambat_sub_kegiatan }}</td>
                                            <td>{{ $laporan->rekomendasi_sub_kegiatan }}</td>
                                            <td>
        ...
        $(document).ready(function() {
            $('#example1').DataTable({
                ...
                columns: [
                    ...
                    {data: "satuan_program"},
                    {data: "target_program"},
                    {data: "realisasi_kinerja_program"},
                    {data: "persen_kinerja_program"},
                    {data: "keterangan_program"},
                    {data: "faktor_pendorong_program"},
                    {data: "faktor_penghambat_program"},
